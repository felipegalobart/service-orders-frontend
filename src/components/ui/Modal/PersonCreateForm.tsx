...
export const PersonCreateForm: React.FC<PersonCreateFormProps> = ({
        ...
        setFormData(prev => ({
            ...
            contacts: [
                ...(prev.contacts || []),
                {
                    ...
                    sector: '',
                    isWhatsApp: false,
                    isDefault: (prev.contacts || []).length === 0 // Primeiro contato é padrão
                }
            ]
        }));
    };
    ...
    const removeContact = (index: number) => {
        setFormData(prev => {
            ...
            const wasRemovingDefault = currentContacts[index]?.isDefault;
            const newContacts = currentContacts.filter((_, i) => i !== index);

            // Se o contato removido era o padrão e ainda há contatos, marcar o primeiro como padrão
            if (wasRemovingDefault && newContacts.length > 0) {
                newContacts[0].isDefault = true;
            }
    ...
    const handleDefaultContact = (index: number, isDefault: boolean) => {
        setFormData(prev => ({
            ...prev,
            contacts: prev.contacts?.map((contact, i) => ({
                ...contact,
                isDefault: i === index ? isDefault : false // Apenas o contato selecionado pode ser padrão
            })) || []
        }));
    };
                                ...
                                <div className="flex items-center">
                                    <div className="relative">
                                        <input
                                            ...
                                            className="w-5 h-5 text-green-600 bg-gray-700 border-2 border-gray-500 rounded focus:ring-green-500 focus:ring-2"
                                        />
                                        {contact.isWhatsApp && (
                                            <svg className="absolute top-0.5 left-0.5 w-4 h-4 text-green-600 pointer-events-none" fill="currentColor" viewBox="0 0 20 20">
                                                <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                                            </svg>
                                        )}
                                    </div>
                                    <label htmlFor={`whatsapp-${index}`} className="ml-3 text-sm font-medium text-gray-300 cursor-pointer hover:text-green-400 transition-colors flex items-center">
                                ...
                                <div className="flex items-center">
                                    <div className="relative">
                                        <input
                                            ...
                                            onChange={(e) => handleDefaultContact(index, e.target.checked)}
                                            className="w-5 h-5 text-blue-600 bg-gray-700 border-2 border-gray-500 rounded focus:ring-blue-500 focus:ring-2"
                                        />
                                        {contact.isDefault && (
                                            <svg className="absolute top-0.5 left-0.5 w-4 h-4 text-blue-600 pointer-events-none" fill="currentColor" viewBox="0 0 20 20">
                                                <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                                            </svg>
                                        )}
                                    </div>
                                    <label htmlFor={`default-${index}`} className="ml-3 text-sm font-medium text-gray-300 cursor-pointer hover:text-blue-400 transition-colors">
                                        ⭐ Contato Padrão
                                    </label>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    {errors[`address_${index}_state`] && (
                                        <p className="mt-1 text-sm text-red-400">{errors[`address_${index}_state`]}</p>
                                    )}
                                </div>
                                <div>
                                    <CEPInput
